		...
		public function __construct(){
			...
			switch($act){
				case "edit":
					...
					$arr = $this->model->get_a_record("select * from tbl_customer where customer_id=$id");
				break;
				case "do_edit":
					...
					$this->model->execute("update tbl_customer set hovaten='$c_fullname' where customer_id=$id");
					...
					if($c_password != ""){
						...
						//update ban ghi co id truyen vao
						$this->model->execute("update tbl_customer_id set
						 password='$c_password' where customer_id=$id");
					}
				...
				case "do_add":
					...
					$c_password = md5($c_password);
					//insert ban ghi
					$this->model->execute("insert into tbl_customer(hovaten,email,password) values('$c_fullname','$c_email','$c_password')");
					//quay tro lai trang user
					header("location:admin.php?controller=user");
				break;
			}
